...
for i in range(len(train_ctrl)):
    ...
    if(np.max(np.abs(Ih_temp))>Ih_temp_max):
        Ih_temp_max=np.max(np.abs(Ih_temp))
...
fig = plt.figure(figsize=(10,4))
plt.subplot(2,1,1)
plt.plot(t[:traintest_cutoff],train_ctrl[:,0], 'g-', label='User Input', linewidth=2, alpha=0.8)
...
#    Ih_temp=u_t+np.matmul(w2_rr,Ih_tm1)
    ...
    if(np.max(np.abs(Ih_temp))>Ih_temp_max1):
        Ih_temp_max1=np.max(np.abs(Ih_temp))
...
fig = plt.figure(figsize=(10,4))
plt.subplot(2,1,1)
plt.plot(t[traintest_cutoff:],test_ctrl[:,0], 'g-', label='User Input', linewidth=2, alpha=0.8)
...
#plt.xlabel('Time', fontsize=12)
plt.ylabel('State Variable :: y1', fontsize=18)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
...
plt.ylabel('State Variable :: y2', fontsize=18)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
#plt.title('Golden Output vs. System Output (During Testing)', fontsize=16)
plt.legend(fontsize=16,loc=1)               